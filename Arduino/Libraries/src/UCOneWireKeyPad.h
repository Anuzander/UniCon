 ...
 *    Ground        1000        3300
 */
#ifndef UCOnewireKeyPad_h
#define UCOnewireKeyPad_h
#include <Arduino.h>
#include "UCBitBool.h"
...
class UCOnewireKeyPad {
  public:
    UCOnewireKeyPad( T &port, char KP[], uint8_t Rows, uint8_t Cols, uint8_t Pin, int R1, int R2 , long Precision)
    ...
    UCOnewireKeyPad( T &port, char KP[], uint8_t Rows, uint8_t Cols, uint8_t Pin, long Precision)
...
template < typename T, unsigned MAX_KEYS >
char UCOnewireKeyPad< T, MAX_KEYS >::Getkey()
{
...
template < typename T, unsigned MAX_KEYS >
uint8_t UCOnewireKeyPad< T, MAX_KEYS >::Key_State()
{
...
template < typename T, unsigned MAX_KEYS >
void UCOnewireKeyPad<T, MAX_KEYS >::LatchKey()
{
  char output[20];
  bool PRINT = false;
  char read = Getkey();
...
template < typename T, unsigned MAX_KEYS >
bool UCOnewireKeyPad< T, MAX_KEYS >::checkLatchedKey(char _key)
{
...
template < typename T, unsigned MAX_KEYS >
void UCOnewireKeyPad< T, MAX_KEYS >::addEventKey(void (*userFunc)(void), char KEY)
{
...
template < typename T, unsigned MAX_KEYS >
void UCOnewireKeyPad< T, MAX_KEYS >::deleteEventKey(char KEY)
{
...
template < typename T, unsigned MAX_KEYS >
void UCOnewireKeyPad< T, MAX_KEYS >::ListenforEventKey()
{
  for (uint8_t idx = 0; idx < Num; idx++)
  {
...
template < typename T, unsigned MAX_KEYS >
void UCOnewireKeyPad< T, MAX_KEYS >::ShowRange()
{
